...
class MessageManager {
    ...
    async loadCurrentUser() {
        try {
            ...
            // 如果未登录，改为提示但不强制跳转：允许展示面向所有人的广播/示例消息
            if (!this.currentUser && window.location.pathname.includes('message.html')) {
                console.warn('未登录用户访问消息页面：不会自动跳转，页面将展示面向所有人的消息');
            }
    ...
    async loadMessages() {
        try {
            ...
            // 如果没有消息数据，初始化示例数据（允许未登录用户看到面向所有人的示例）
            if (this.messages.length === 0) {
                await this.initSampleMessages();

                // 重新加载刚刚创建的示例数据
                if (window.dataManager && window.dataManager.isInitialized) {
                    const messagesData2 = window.dataManager.folder.getAll('messages');
                    this.messages = Array.isArray(messagesData2) ? messagesData2 : [];
                } else {
                    const savedMessages2 = localStorage.getItem('messages');
                    this.messages = savedMessages2 ? JSON.parse(savedMessages2) : [];
                }
            }
    ...
    async initSampleMessages() {
        try {
            ...
            const recipient = this.currentUser && this.currentUser.id ? this.currentUser.id : 'all';
            ...
            const sampleMessages = [
                ...
                },
                {
                    ...
                    relatedItem: {
                        ...
                        authorName: this.currentUser ? this.currentUser.username : 'video_creator'
                    }
                },
                {
                    id: 'msg_3',
                ...
                },
                {
                    ...
                    senderName: 'video_creator',
                    recipientId: recipient,
                    content: this.currentUser ? ('在动态中提到了您：快来看看这个有趣的视频！@' + this.currentUser.username) : '在动态中提到了您：快来看看这个有趣的视频！',
                    timestamp: new Date(Date.now() - 6 * 60 * 60 * 1000).toISOString(),
                    read: false,
                    relatedItem: {
                        type: 'dynamic',
                        id: 'dynamic_1',
                        content: this.currentUser ? ('分享了一个超棒的视频教程，快来学习吧！@' + this.currentUser.username) : '分享了一个超棒的视频教程，快来学习吧！'
                    }
                }
            ];
    ...
    updateUI() {
        if (!this.currentUser) {
            console.warn('未登录用户，仍然显示面向所有人的消息');
        }
    ...
    getUserMessages() {
        if (!this.currentUser) {
            // 未登录用户只能看到面向所有人的消息或没有指定接收人的系统消息
            return this.messages.filter(msg => msg.recipientId === 'all' || (msg.type === 'system' && !msg.recipientId));
        }

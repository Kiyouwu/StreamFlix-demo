...
class DynamicManager {
    ...
    updateComposerInfo() {
        ...
        // 兼容性保护：如果用户没有头像或头像无效，使用默认占位图并添加错误回退
        if (composerAvatar) {
            try {
                composerAvatar.src = (this.currentUser && this.currentUser.avatar) ? this.currentUser.avatar : 'assets/default-avatar.png';
            } catch (e) {
                composerAvatar.src = 'assets/default-avatar.png';
            }
            // 避免外部图片 404 导致空白，添加 onerror 回退
            composerAvatar.onerror = function() { this.onerror = null; this.src = 'assets/default-avatar.png'; };
        }

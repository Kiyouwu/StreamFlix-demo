...
class VideoPlayManager {
    ...
    async loadVideo() {
        ...
        if (!effectiveVideoId) {
            // 回退至第一个示例视频（如果存在），便于直接打开页面播放 demo
            try {
                const allVideos = dataManager.folder.getAllItems('videos');
                if (Array.isArray(allVideos) && allVideos.length > 0) {
                    effectiveVideoId = allVideos[0].id;
                    console.log('没有传入 id，回退到第一个视频:', effectiveVideoId);
                }
            } catch (e) {
                console.warn('获取视频列表失败，无法回退:', e);
            }
        }
        ...
        try {
            this.currentVideo = dataManager.video.getById(effectiveVideoId);
    ...
    createLikeMessage() {
        try {
            ...
            if (window.messageManager && typeof window.messageManager.createMessage === 'function') {
                window.messageManager.createMessage(messageData);
                return;
            }

            // 其次使用 dataManager 的 folder 保存到 StreamFlix_data/messages
            if (window.dataManager && dataManager.isInitialized) {
                try {
                    const msgs = dataManager.folder.getAll('messages') || [];
                    const newMessage = Object.assign({ id: 'msg_' + Date.now() + '_' + Math.random().toString(36).substr(2,9), timestamp: new Date().toISOString(), read: false }, messageData);
                    msgs.push(newMessage);
                    dataManager.folder.save('messages', msgs);
                    // 通知全局更新
                    window.dispatchEvent(new CustomEvent('globalMessagesUpdated'));
                    return;
                } catch (e) {
                    console.warn('通过 DataManager 保存点赞消息失败，回退到 localStorage', e);
                }
            }
            ...
            const likeMessage = Object.assign({ id: 'msg_' + Date.now() + '_' + Math.random().toString(36).substr(2,9), timestamp: new Date().toISOString(), read: false }, messageData);
    ...
    createCommentMessage(commentContent) {
        try {
            const messageData = {
                ...
                relatedItem: { type: 'video', id: this.currentVideo.id, title: this.currentVideo.title, cover: this.currentVideo.cover }
            };
            ...
            if (window.dataManager && dataManager.isInitialized) {
                try {
                    ...
                    console.warn('通过 DataManager 保存评论消息失败，回退到 localStorage', e);
                }
            }

            const messages = (typeof safeJsonParse === 'function') ? safeJsonParse(localStorage.getItem('messages'), []) : JSON.parse(localStorage.getItem('messages') || '[]');
            const commentMessage = Object.assign({ id: 'msg_' + Date.now() + '_' + Math.random().toString(36).substr(2,9), timestamp: new Date().toISOString(), read: false }, messageData);
    ...
    createNotification(targetUserId, title, content, meta = {}) {
        try {
            const messageData = {
                type: 'notification',
                senderId: this.currentUser ? this.currentUser.id : null,
                senderName: this.currentUser ? this.currentUser.username : '系统',
            ...
            if (window.dataManager && dataManager.isInitialized) {
                try {
                    const msgs = dataManager.folder.getAll('messages') || [];
                    const newMsg = Object.assign({ id: 'msg_' + Date.now() + '_' + Math.random().toString(36).substr(2,9), timestamp: new Date().toISOString(), read: false }, messageData);
                    msgs.push(newMsg);
                    dataManager.folder.save('messages', msgs);
                    window.dispatchEvent(new CustomEvent('globalMessagesUpdated'));
                    return;
                } catch (e) {
                    console.warn('通过 DataManager 保存通知失败，回退到 localStorage', e);
                }
            }
            ...
            try {
                ...
                const fallback = Object.assign({ id: 'msg_' + Date.now() + '_' + Math.random().toString(36).substr(2,9), timestamp: new Date().toISOString(), read: false }, messageData);
                arr.push(fallback);
                localStorage.setItem('messages', JSON.stringify(arr));
                window.dispatchEvent(new CustomEvent('globalMessagesUpdated', { detail: { message: fallback } }));
            } catch (e) {
    ...
    loadRecommendedVideos() {
        ...
        try {
            ...
            recommendedVideos.forEach(video => {
                ...
                img.alt = video.title || '';
                // 如果封面地址安全且存在则使用；否则直接使用默认封面，避免空 src 导致显示失败
                if (isSafeUrl(video.cover)) {
                    img.src = video.cover;
                } else {
                    img.src = 'assets/demoCover.png';
                }
                // 再加一层错误回退（例如远程图片 404）
                img.addEventListener('error', () => { if (img.src.indexOf('demoCover.png') === -1) img.src = 'assets/demoCover.png'; });
